    ...
    for attempt in range(tries):
        try:
            ...
            if r.status_code in (429, 430, 503, 403):
                ...
                time.sleep(20 + random.randint(0, 15)); continue
...
def parse_posts(html):
    ...
    for chunk in re.split(r"<tr\b", html):
        ...
        out.append({"srl": sm.group(1), "title": strip_tags(am.group(1)),
...
def check_member(state, name, mid, srl):
    ...
    if last == 0:
        state[key] = newest; print(f"[{name}] 감시 시작 (기준 {newest})"); return
    new_posts = [p for p in posts if int(p["srl"]) > last]
    if not new_posts:
        print(f"[{name}] 새 글 없음"); return
    sent, ok = 0, True
    for p in new_posts[-MAX_MESSAGES_PER_MEMBER:]:
        try:
            send_telegram(build_message(name, p)); sent += 1
        except Exception as e:
            print(f"[{name}] 전송 실패: {e}"); ok = False; break
    if ok:
        state[key] = newest; print(f"[{name}] 새 글 {sent}건 전송!")
...
def build_schedule():
    ...
    for j, m in enumerate(prios):
        for k in range(PRIORITY_WEIGHT):
            slots.append((((k + (j + 0.5) / max(1, len(prios))) / PRIORITY_WEIGHT) % 1.0, m))
...
def check_once():
    ...
    for i, (name, mid, srl) in enumerate(MEMBERS):
        if i > 0: time.sleep(MEMBER_GAP_SEC + random.randint(0, 15))
        check_member(state, name, mid, srl); save_state(state)


def watch():
    schedule = build_schedule()
    ng = round(len(schedule) * (MEMBER_GAP_SEC + 7) / 60)
    pg = round(len(schedule) / PRIORITY_WEIGHT * (MEMBER_GAP_SEC + 7) / 60, 1)
    print(f"감시 모드 시작 — 한 바퀴 {len(schedule)}칸 (우선순위 ~{pg}분 / 일반 ~{ng}분). Ctrl+C 중지")
    try:
        send_telegram(f"✅ fmkorea 알리미 감시 시작! (총 {len(MEMBERS)}명 감시)\n새 글이 올라오면 알려드릴게요.")
    except Exception as e:
        print(f"시작 메시지 전송 실패: {e}")
    state = load_state(); idx = 0
    while True:
        name, mid, srl = schedule[idx % len(schedule)]; idx += 1
        try:
            check_member(state, name, mid, srl); save_state(state)
        except Exception as e:
            print(f"[{name}] 예외: {e}")
