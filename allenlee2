...
실행:
  python allenlee2.py          한 번만 확인 (테스트)
  python allenlee2.py watch    10분마다 계속 감시
"""
...
DELAY_BETWEEN_MEMBERS_SEC = 25   # fmkorea 유저 간 간격 (차단 방지)
    ...
    for attempt in range(tries):
        ...
        try:
            ...
            if e.code in (429, 430, 503, 403):
                wait = 30 * (attempt + 1) + random.randint(0, 15)
                print(f"    (차단 {e.code} — {wait}초 쉬었다 재시도)")
                time.sleep(wait)
                continue
            raise
...
def check_fmkorea(state, first_run_names):
    for i, (name, mid, srl) in enumerate(MEMBERS):
        if i > 0:
            time.sleep(DELAY_BETWEEN_MEMBERS_SEC + random.randint(0, 10))
        key = "fm_" + srl
        try:
            ...
            continue
        if not posts:
            print(f"[{name}] 글 목록 못 찾음 — 다음 확인 때 재시도")
            continue
...
def check_once():
    ...
    check_fmkorea(state, first_run_names)
    ...
    if first_run_names or tooja_first:
        parts = []
        if first_run_names:
...
def main():
    if len(sys.argv) > 1 and sys.argv[1].lower() == "watch":
        print(f"감시 모드 시작 — fmkorea {len(MEMBERS)}명 + tooja 포지션, "
              f"{CHECK_INTERVAL_SEC//60}분마다 확인합니다. (Ctrl+C 로 중지)")
        while True:
            try:
                check_once()
            except Exception as e:
                print(f"[오류] {e} — 다음 확인 때 재시도")
            print(f"--- {CHECK_INTERVAL_SEC//60}분 대기 ---")
